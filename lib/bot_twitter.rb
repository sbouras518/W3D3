...
# Envoyer une blague à 5 journalistes au hasard.
def hello(bonjour_monde)
	...
	# .sample prend 5 comptes au hasard dans l'array.
	random = journalists.sample(5)

	# Pour chaque comptes on lui envoie individuellement le message.
	random.each do |journalists_choosen|
		@client.update("#{journalists_choosen} #bonjour_monde")
	end
end

# Like les (25) derniers tweets contenant le #bonjour_monde.
def like_hello(client)

	# .search va chercher le #bonjour_monde dans les 25 derniers tweets,
	# le tweet les collecte puis on like avec le .favorite.
	@client.search("#bonjour_monde -rt", result_type: "recent").take(25).collect do |tweet|
		@client.favorite(tweet)
	end
	 
end

def follow_hello(follow)

	# .search va chercher le #bonjour_monde dans les 5 derniers tweets,
	# le i les collecte puis on follow avec le .follow
